...
export const ConfirmForm = (props: ConfirmFormProps) => {
  ...
  const submitFunction = async (e: SyntheticEvent) => {
    ...
    setError("");
    setLoading(true);
    const response = await apiClient.organizer.confirm(props.id.toString(), {
      headers: { code: code },
    });
    setLoading(false);
    if (response.ok) {
      ...
      if (response.status === 400) setError("Invalid verification code.");
  ...
  return (
    <form onSubmit={submitFunction}>
      <Stack spacing={2}>
        ...
        <Typography>We sent a verification code to your inbox.</Typography>
        <TextField
