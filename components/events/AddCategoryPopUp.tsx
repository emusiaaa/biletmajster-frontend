...
export default function AddCategoryPopUp(props: CatProps) {
  ...
  const addNewCategory = async () => {
    ...
    if (sessionToken !== undefined) {
      const response = await apiClient.categories.addCategories({
        headers: { sessionToken: sessionToken, categoryName: category },
      });
      if (response.ok) {
        console.log("juhuu");
        ...
        if (response.status === 400) {
          ...
          alert(
            "Could not add category. Please refresh page.\n Received error: " +
              response.statusText
          );
      }
    } else {
  ...
  return (
    <div>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          <DialogContentText>
            To add a new event category to this website, please enter the
        ...
        <DialogActions>
          <Button onClick={handleClose} data-testid="cancel-btn">
            Cancel
          </Button>
          <Button onClick={addNewCategory} data-testid="add-category-btn">
            Add
          </Button>
        </DialogActions>
      </Dialog>
