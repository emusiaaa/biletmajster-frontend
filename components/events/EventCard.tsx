        ...
        <>
            {
                props.status === EventStatus.Pending ?
                    <Chip style={{backgroundColor:'#FFCA3A', height:'25px'}} icon={<MoreHorizIcon style={{ color: 'black' }}/>} label={props.status} /> :
                    props.status === EventStatus.InFuture ?
                    <Chip style={{backgroundColor:'#D972FF', height:'25px'}} icon={<UpdateOutlinedIcon style={{ color: 'black' }}/>} label={props.status} /> :
                        props.status === EventStatus.Cancelled ?
                            <Chip style={{backgroundColor:'red', height:'25px'}} icon={<HighlightOffIcon style={{ color: 'black' }}/>} label={props.status} /> :
                            //EventStatus.Done
                            <Chip style={{backgroundColor:'#8AC926', height:'25px'}} icon={<CheckCircleOutlineIcon style={{ color: 'black' }}/>} label={props.status} />
            }
        </>
    )
}
...
export const EventCard = (props: EventProps) => {
    ...
    const handleDelete = async () => {
        ...
        if (sessionToken !== undefined) {
            const response = await apiClient.events.cancelEvent(props.event.id.toString(),{ headers: { sessionToken : sessionToken }});
            if (response.ok) {
        ...
        <>
            <Card sx={{ maxWidth: 800, mt:5, padding:3,  boxShadow: 13}}>
                <CardHeader
                    ...
                    title={<Title title={props.event.title} status={props.event.status}/>}
