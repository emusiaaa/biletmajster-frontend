...
export class EventValidator extends Validator<EventForm> {
  constructor() {
    ...
    this.ruleFor("title").notEmpty().withMessage("Title cannot be empty");

    this.ruleFor("name").notEmpty().withMessage("Name cannot be empty");

    this.ruleFor("latitude").notEmpty().withMessage("Latitude cannot be empty");
