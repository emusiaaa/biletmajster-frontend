...
const setup = () => {
  ...
  document.cookie = "backendUrl=sampleUrl; expires=Thu, 18 Dec 2033 12:00:00 UTC; path=/"; 
  document.cookie = "sessionToken=sampleToken; expires=Thu, 18 Dec 2033 12:00:00 UTC; path=/"; 
  ...
  const TestComponent = () => {
    ...
    return (
      <>
        ...
        <input data-testid="urlChange" value={url} onChange={e => setUrl(e.target.value)} />
        <input data-testid="tokenChange" value={token ?? ""} onChange={e => setToken(e.target.value)} />
      </>
    )
  }
...
describe('Cookie', () => {
  it('is properly read', () => {
    ...
    it('is properly modified', async () => {
      ...
      expect(document.cookie.includes("sessionToken=modifiedToken")).toBeTruthy();
    })
})
