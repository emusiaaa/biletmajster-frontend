...
describe("AddCategoryPopUp", () => {
  it("renders empty category form", async () => {
    ...
    it("continues when correct category provided", async () => {
      (apiClient.categories.addCategories as any).mockImplementation(
        (requestParams: {
          headers: { sessionToken: string; categoryName: string };
        }) => {
  ...
  it("detects empty category", async () => {
    ...
    expect(
      screen.getAllByText((text: string) =>
        text.includes("Please type sth.....")
      )
    ).toHaveLength(1);
  });
});
