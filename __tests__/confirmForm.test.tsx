...
const setup = () => {
  ...
  render(
    <RecoilRoot>
      <ConfirmForm id={100} goToPrevious={prevStepMock} />
    </RecoilRoot>
  );
...
describe("ConfirmForm", () => {
  it("renders empty confirmation form", () => {
    ...
    it("shows message if code is correct", async () => {
      (apiClient.organizer.confirm as any).mockImplementation(
        (id: string, params: { headers: { code: string } }) => {
          expect(id).toBe("100");
