...
const setup = async () => {
  ...
  const titleInput = (await screen.findByTestId("title-input")).querySelector(
    "input"
  )!;
  const nameInput = (await screen.findByTestId("name-input")).querySelector(
    "textarea"
  )!;
  const longInput = (await screen.findByTestId("long-input")).querySelector(
    "input"
  )!;
  const latInput = (await screen.findByTestId("lat-input")).querySelector(
    "input"
  )!;
  const maxInput = (await screen.findByTestId("max-input")).querySelector(
    "input"
  )!;
