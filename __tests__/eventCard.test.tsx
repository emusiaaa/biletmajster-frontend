...
const setup = async () => {
    ...
    const progressBarElement = (await screen.findByTestId('progress-bar')).querySelector('.MuiLinearProgress-bar');
    return {
        user, cardHeader, categoryElements, editButton, deleteButton, freePlacesElement, progressBarElement, nextStepMock
    };
}

describe('EventCard', () => {
    it('renders event card with data', async () => {
        const { user, cardHeader, categoryElements, editButton, deleteButton, freePlacesElement, progressBarElement, nextStepMock } = await setup();
